...
def selkov_system(state, t, b):
    """
    Selkov glycolysis system:
    dx/dt = -x + ay + x^2 y
    dy/dt = b - ay - x^2 y
    """
    a = 0.1
...
num_test_steps = int(T_test / delta_t)
bif_relevant_dim = 0
...
cps_id = np.linspace(b_train_min, b_train_max, num_subjects)
# Keep extrapolation strictly below the training b domain.
cps_ood = np.linspace(b_extrap_min, b_train_min, num_ood_subjects + 1)[:-1]
cps = np.concatenate([cps_ood, cps_id])

ICs_train = np.random.uniform(low=0.1, high=2.0, size=(num_subjects, num_ICs, 2))
ICs_test_id = np.random.uniform(low=0.1, high=2.0, size=(num_subjects, num_ICs, 2))
ICs_test_ood = np.random.uniform(low=0.1, high=2.0, size=(num_ood_subjects, num_ICs, 2))
...
X_train = np.zeros((cps_id.shape[0], num_ICs, num_train_steps, 2))  # (num_subjects, num_ts_per_subject, T_train, N)
for i, cp in enumerate(cps_id):
    for j in range(num_ICs):
        IC = ICs_train[i, j, :]
        X_train[i, j, :, :] = odeint(selkov_system, IC, t_train, args=(cp,))
print(f"X_train shape: {X_train.shape}")

# integrating in-domain test data
X_id = np.zeros((cps_id.shape[0], num_ICs, num_test_steps, 2))  # (num_subjects, num_ts_per_subject, T_test, N)
for i, cp in enumerate(cps_id):
    for j in range(num_ICs):
        IC = ICs_test_id[i, j, :]
        X_id[i, j, :, :] = odeint(selkov_system, IC, t_test, args=(cp,))
print(f"X_id shape: {X_id.shape}")

# integrating out-of-domain test data
X_ood = np.zeros((cps_ood.shape[0], num_ICs, num_test_steps, 2))  # (num_subjects, num_ts_per_subject, T_test, N)
for i, cp in enumerate(cps_ood):
    for j in range(num_ICs):
        IC = ICs_test_ood[i, j, :]
        X_ood[i, j, :, :] = odeint(selkov_system, IC, t_test, args=(cp,))
...
# saving data
np.savez(
    'selkov_dataset_standardized.npz',
...
# plotting trajectories for all b values
fig, axes = plt.subplots(5, 8, figsize=(24, 15))
axes = axes.flatten()

for idx, cp in enumerate(cps):
    ax = axes[idx]
    if idx < cps_ood.shape[0]:  # OOD
        ood_idx = idx
        for j in range(num_ICs):
            traj = X_ood_standardized[ood_idx, j, :, :]
            ax.plot(traj[:, 0], traj[:, 1], color='gray', alpha=0.5, linewidth=0.8)
        ax.set_title(f'b={cp:.2f} (OOD)', fontsize=9)
    else:  # ID
        id_idx = idx - cps_ood.shape[0]
        for j in range(num_ICs):
        ...
        ax.set_title(f'b={cp:.2f} (ID)', fontsize=9)
    ax.set_xlabel('x', fontsize=8)
    ax.set_ylabel('y', fontsize=8)
    ax.tick_params(labelsize=7)

plt.suptitle('Selkov Trajectories (blue=train, gray=test)', fontsize=12)
plt.tight_layout()
plt.savefig('selkov_trajectories.png', dpi=100)
plt.show()
